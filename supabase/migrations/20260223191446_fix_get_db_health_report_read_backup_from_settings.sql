/*
  # Fix get_db_health_report: read last backup date from settings table

  The function was querying a non-existent "backups" table.
  The actual last backup timestamp is stored in settings.last_backup_date.
  This fix reads from settings instead, so the health check correctly
  shows the last backup date after a server backup is created.
*/
