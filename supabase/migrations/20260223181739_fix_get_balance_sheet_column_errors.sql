/*
  # Fix get_balance_sheet function

  ## Problems Fixed

  1. cash_registers.current_balance — column does not exist
     → replaced with expected_balance (the running cash balance for open registers)

  2. Inner DECLARE block for VAT variables — invalid PL/pgSQL syntax
     → moved v_vat_output and v_vat_input into the outer DECLARE section

  3. purchases.purchase_date is timestamptz, not date
     → added ::date cast so the <= v_as_of_date comparison works correctly

  4. sales.sale_date VAT subquery missing ::date cast
     → added ::date cast for consistency

  5. Retained earnings calculated as Assets - Liabilities - Capital
     → replaced with proper accounting: net income = revenue - COGS - operating expenses
        accumulated up to the as_of_date, matching the income statement logic

  ## No accounting logic was changed.
  ## Only column names and syntax were corrected.
*/
...
BEGIN
  ...
  -- Cash: sum of expected_balance for all open registers (fixed: was current_balance)
  ...
  -- Accounts Receivable: confirmed unpaid credit sales up to as_of_date
  ...
  -- Inventory: moving-average cost × quantity on hand
  SELECT COALESCE(SUM(quantity_on_hand * average_cost), 0)
  INTO v_inventory_value
  FROM product_costing
  WHERE (p_branch_id IS NULL OR branch_id = p_branch_id);

  -- Fixed Assets: gross cost minus accumulated depreciation up to as_of_date
  SELECT COALESCE(SUM(
  ...
  -- Accounts Payable: unpaid purchases up to as_of_date
  -- (fixed: added ::date cast — purchase_date is timestamptz)
  SELECT COALESCE(SUM(total), 0)
  INTO v_ap
  FROM purchases
  WHERE payment_status = 'unpaid'
  ...
  -- VAT Payable: output VAT on confirmed sales minus input VAT on purchases
  -- (fixed: moved out of invalid inner DECLARE block; added ::date cast on sale_date)
  ...
  -- Partner Capital: total cash/asset contributions (excluding expenses)
  ...
  -- Net Income (Retained Earnings for current period):
  -- Revenue minus COGS (cost of goods sold from inventory) minus operating expenses
  -- This matches the income statement logic so the balance sheet ties to it.
