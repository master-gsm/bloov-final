/*
  # Handle cancelled sales cash reversal

  ## Summary
  When a cash sale is cancelled, the corresponding cash transaction should be reversed.
  This ensures the cash register balance remains accurate when sales are cancelled.

  ## Changes
  - Update `record_sale_cash_movement()` to also handle 'cancelled' status
  - Removes the register transaction when a confirmed cash sale is cancelled
*/
...
DECLARE
  v_register_id uuid;
BEGIN
  -- Handle returned/cancelled status: remove the cash movement
