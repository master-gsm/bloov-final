/*
  # Fix journal entry creation to be idempotent

  ## Summary
  The `create_sale_journal_entry` function was creating journal entries every time it was called,
  but the unique constraint prevented duplicate entries for the same sale. This caused errors
  when trying to restore a sale. The function now checks if an entry exists before creating one.

  ## Changes
  - Update `create_sale_journal_entry()` to check if journal entry already exists for the sale
  - If exists, return existing entry ID instead of creating duplicate
  - Ensures sales can be restored without journal entry errors
*/
