...
/*
  # Fix create_sale_atomic: Add sequential line_number to journal_lines inserts

  ## Problem
  journal_lines.line_number has a NOT NULL constraint, but create_sale_atomic
  was inserting rows without providing this value.

  ## Fix
  Add a v_line_num counter variable, reset it to 1 before the journal lines
  block, and increment it for each INSERT into journal_lines.

  ## No changes to any accounting logic, amounts, constraints, or other operations.
*/
...
-- Step 1: Insert journal entry as Draft
...
-- Step 2: Insert all journal lines with sequential line_number
v_line_num := 1;
...
-- Step 3: Post the journal entry after all lines are inserted
