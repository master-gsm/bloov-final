/*
  # Fix process_purchase_receipt_atomic: align with actual vat_transactions schema

  The vat_transactions table uses:
    - source_type / source_id  (not reference_type / reference_id — those exist as extras)
    - direction ('input' | 'output')  (not transaction_type)
    - tax_code, tax_rate, vat_category
    - period_month, period_year
    - status ('pending' | 'filed')
    - supplier_id
    - invoice_number

  Also: journal_lines uses (id, journal_entry_id, account_id, debit, credit, description)
  without the unique constraint on line_number in some versions — we insert with explicit line_number.
*/
...
BEGIN
  ...
  IF v_purchase.status = 'received' OR EXISTS (
    ...
    -- Already processed — ensure status is correct and return
  ...
  IF v_has_vat THEN
    INSERT INTO vat_transactions (
      ...
      v_purchase_date, v_branch_id, 'pending',
      'purchase', p_purchase_id,
