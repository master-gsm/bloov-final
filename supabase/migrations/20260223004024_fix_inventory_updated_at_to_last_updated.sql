...
/*
  # Fix inventory column name in create_sale_atomic

  ## Problem
  The function create_sale_atomic uses `updated_at` when updating the inventory table,
  but the actual column name in the inventory table is `last_updated`.

  ## Change
  - In the UPDATE inventory statement inside create_sale_atomic, replace `updated_at = now()` with `last_updated = now()`

  ## Scope
  - Only the inventory UPDATE statement is changed
  - No other logic, columns, or tables are modified
*/
