/*
  # إرجاع View الحساب الجاري لقراءة من operating_expenses
  
  ## التوضيح
  - جدول `expenses` يحتوي فقط على salaries وcommissions
  - جدول `operating_expenses` يحتوي على المصاريف التشغيلية الفعلية
  - المصاريف التي أضافها المستخدم موجودة في `operating_expenses`
  
  ## الحل النهائي
  - القراءة من `operating_expenses` 
  - شمول جميع الأنواع ما عدا 'capital'
  - استخدام `net_amount` بدلاً من `amount`
*/

-- Drop existing view
DROP VIEW IF EXISTS v_partner_analytical_balances CASCADE;

-- Recreate with correct table
...
COMMENT ON VIEW v_partner_analytical_balances IS 'الحساب الجاري التحليلي للشركاء - يحسب رصيد كل شريك بناءً على جميع المصاريف المشتركة (من operating_expenses) والحصة المفترضة والتسويات';
