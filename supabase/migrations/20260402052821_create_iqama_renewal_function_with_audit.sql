/*
  # Create Iqama Renewal Function with Audit Logging

  1. New Function
    - `fn_renew_iqama` - Renews an employee's iqama (residence permit)
    - Calculates new expiry from the ORIGINAL expiry date (not today)
    - Supports preset durations (3/6/12 months) or custom date
    - Logs every renewal in audit_logs for compliance
    - Restricted to super_admin and admin roles only

  2. Parameters
    - p_employee_id (UUID) - The employee to renew
    - p_duration_months (INTEGER) - 3, 6, or 12 months (NULL if custom date)
    - p_custom_date (DATE) - Custom new expiry date (NULL if using duration)

  3. Security
    - Only super_admin and admin can execute
    - Full audit trail with old/new dates, user, and duration

  4. Important Notes
    - New expiry is calculated from OLD expiry date, not current date
    - If old expiry is NULL, calculates from today as fallback
    - Returns JSON with old/new dates and days remaining
*/
...
BEGIN
  ...
  SELECT id, full_name, full_name_ar, iqama_number, iqama_expiry_date, branch_id, company_id
  INTO v_employee
  FROM employees
...
COMMENT ON FUNCTION fn_renew_iqama IS 'Renews employee iqama from original expiry date. Restricted to admin/super_admin with full audit logging.';
