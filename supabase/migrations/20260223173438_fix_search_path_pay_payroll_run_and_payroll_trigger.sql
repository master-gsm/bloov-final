/*
  ...
  ## Changes
  - public.pay_payroll_run: already had SECURITY DEFINER, adds SET search_path = public
  - public.create_journal_entry_on_payroll_paid: adds SECURITY DEFINER + SET search_path = public

  No business logic or function body changes.
*/

-- 1. pay_payroll_run — add SET search_path (body unchanged)
...
-- Mapped payment method for expenses table (only cash/transfer/card allowed)
...
-- IDEMPOTENCY: skip GL if already posted for this run
IF EXISTS (
...
-- Map payment method to expenses-table-compatible value
...
-- 2. create_journal_entry_on_payroll_paid — add SECURITY DEFINER + SET search_path (body unchanged)
CREATE OR REPLACE FUNCTION public.create_journal_entry_on_payroll_paid()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
