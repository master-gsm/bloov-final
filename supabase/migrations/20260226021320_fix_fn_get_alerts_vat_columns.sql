/*
  # Fix fn_get_alerts VAT return column references

  1. Changes
    - Replace `vr.period_start` with `vr.date_from` (correct column name)
    - Replace `vr.period_end` with `vr.date_to` (correct column name)
    - Replace `vr.net_vat_due` with `vr.net_vat_payable` (correct column name)

  2. Notes
    - The vat_returns table uses `date_from`/`date_to` not `period_start`/`period_end`
    - The net amount column is `net_vat_payable` not `net_vat_due`
*/
...
v_branch_id uuid;
BEGIN
SELECT u.branch_id INTO v_branch_id FROM users u WHERE u.id = auth.uid();
...
'VAT return for period ' || to_char(vr.date_from, 'Mon YYYY') || ' has a net balance of ' || vr.net_vat_payable::text || ' SAR.',
'الإقرار الضريبي للفترة ' || to_char(vr.date_from, 'Mon YYYY') || ' يحتوي على رصيد صافي ' || vr.net_vat_payable::text || ' ر.س.',
'vat_returns'::text,
vr.id::text,
now()
FROM vat_returns vr
WHERE vr.status IN ('draft', 'submitted')
AND vr.net_vat_payable != 0
AND vr.date_to < now() - INTERVAL '15 days'
LIMIT 5;
END IF;
