    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen flex items-center justify-center app-background p-4">
          <div className="bg-dark-surface backdrop-blur-xl rounded-2xl shadow-dark-xl p-8 max-w-md w-full text-center space-y-4 border border-dark-border">
            <div className="w-16 h-16 bg-red-500/10 rounded-full flex items-center justify-center mx-auto">
              <svg className="w-8 h-8 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01M12 3a9 9 0 110 18A9 9 0 0112 3z" />
              </svg>
            </div>
            <h2 className="text-xl font-bold text-mauve-50">حدث خطأ غير متوقع</h2>
            <p className="text-mauve-400 text-sm">{this.state.error?.message || 'Unknown error'}</p>
            <button
              onClick={() => this.setState({ hasError: false, error: null })}
              className="px-6 py-2.5 bg-accent text-white rounded-xl hover:bg-accent-hover transition-all font-medium shadow-glow-sm"
            >
              المحاولة مجددا
            </button>
          </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center login-background">
        <div className="text-center">
          ...
          <p className="mt-4 text-mauve-300 font-medium">{t('common.loading')}</p>
        </div>
      </div>
    );
  }
      ...
      return (
        <div className="flex items-center justify-center h-full">
          <div className="text-center p-8">
            ...
            <h3 className="text-lg font-semibold text-mauve-50 mb-1">لا توجد صلاحية</h3>
            <p className="text-mauve-400 text-sm">ليس لديك صلاحية للوصول لهذا القسم</p>
          </div>
        </div>
      );
    }
